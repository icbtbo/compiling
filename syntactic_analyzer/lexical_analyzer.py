...
class Token(object):
    ...
    def __init__(this): 
        ...
        this.patterns = re.compile('|'.join([Annotation, Keyword, Method, ID, Number, Operator,Separator,  Error])) 
    ...
    def run(this, line, flag=True):
        for token in this.get_token(line):
            if flag:
                ...
                this.word_list.append(token)

def getwords():
    token = Token()
    filepath = "./test.txt" #要分析的代码所在的文件路径
